#!/usr/bin/env bash
# Helper: produce one library's pipelines/ package.
# Args: $1 = library name (svt / lvt / ulvt / ...)
set -e
LIB=$1
DIR=$(dirname "$0")/$LIB
mkdir -p "$DIR/pipelines"

cat > "$DIR/pipelines/__init__.py" <<PY
from .asset import defs

__all__ = ["defs"]
PY

cat > "$DIR/pipelines/asset.py" <<PY
"""Lesson 12 multi_location — library '${LIB}'.
