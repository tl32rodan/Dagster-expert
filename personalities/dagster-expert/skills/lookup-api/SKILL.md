...
```bash
# Topic known
Read personalities/dagster-expert/database/dagster-1.13.3/docs/<topic>.md

# Topic unknown — start with INDEX
Read personalities/dagster-expert/database/dagster-1.13.3/docs/INDEX.md
```
...
```bash
ls personalities/dagster-expert/database/dagster-1.13.3/examples/
Read personalities/dagster-expert/database/dagster-1.13.3/examples/<NN>_*.py
```
...
The `/search` skill (project-wide, provided by the `database`
capability) routes to SMAK against the **active personality**'s
workspaces. Make sure `dagster-expert` is the active personality first
(check `MEMORY.md`'s `> **Active personality**:` callout).

Equivalent direct SMAK call if you need to bypass the slash command:
```
smak search "data version propagation when upstream changes" \
  --config personalities/dagster-expert/database/dagster-1.13.3/config.yaml \
  --top-k 5 --json
```
