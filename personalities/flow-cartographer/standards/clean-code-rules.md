<!-- all-might generated -->
# Clean-code rules — optional code-quality standard for the verify tick

Seven mechanical checks the `verify` tick MAY run on an increment's
changed lines (using the finding format in
`standards/refusal-patterns.md`), producing one finding per violation.
These are no longer a mandatory gate — the six verify checks in
`skills/verify-loop/SKILL.md` are. Apply them when an increment's code
is substantial enough to warrant it; don't "weigh tradeoffs", just flag
violations.
...
REJECT: <rule-id>:
  ...
  Source: personalities/flow-cartographer/standards/clean-code-rules.md §<rule>
```
