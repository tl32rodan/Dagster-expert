<!-- all-might generated -->
# TDD rules — optional code-quality standard for the verify tick

This file defines **mechanical** TDD requirements the `verify` tick MAY
apply to an increment's code (e.g. a large factory change), using the
finding format in `standards/refusal-patterns.md`. It is no longer a
mandatory gate — the six verify checks in `skills/verify-loop/SKILL.md`
are. Use the checks below to produce line-level findings when warranted.
