"""M1 — Pydantic spec schema (WHITEPAPER §3.1).

Strict validation at load time: a malformed spec fails here, never at
runtime. Only one execution path exists now (Execution Fabric); no
`dispatch` or `trigger` configurability — there is one mode.
"""
...
class AssetSpec(BaseModel):
    ...
    version: str | None = None            # base-version name or "module:callable"
    ...
    lsf: LSFResource | None = None        # compute assets: override spec.lsf.default
...
class FlowSpec(BaseModel):
    ...
    def effective_lsf(self, a: AssetSpec) -> LSFResource:
        """Compute assets MUST have an LSF resource (validation enforces)."""
        return a.lsf or self.lsf.get("default") or LSFResource()
