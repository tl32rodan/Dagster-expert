...
cd personalities/dagster-librarian/database/dagster-1.13.3/examples
...
```python
Read personalities/dagster-librarian/database/dagster-1.13.3/docs/data-version-and-staleness.md
```

**Semantic search** (when topic is fuzzy):
```python
mcp__smak__search query="data version propagation in Style B with explicit deps"
# → returns ranked entries with snippets
```
